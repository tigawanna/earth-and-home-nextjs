...
import { betterUploadS3 } from "@/app/api/upload/route";
import fs from "fs/promises";

async function listObjects(bucketName: string) {
  // betterUploadS3.send(new ListBucketsCommand({}))
  const data = await betterUploadS3.send(new ListObjectsV2Command({ Bucket: bucketName }));
...
async function uploadImage() {
  ...
  await betterUploadS3.send(new PutObjectCommand({
